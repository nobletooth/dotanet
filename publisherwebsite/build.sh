...
if [ $? -eq 0 ]; then
  ...
  scp $OUTPUT_BINARY $SERVER:~/publisherwebsite/
  if [ $? -eq 0 ]; then
    echo "Binary successfully copied to server"

    # Run the publisherwebsite
    echo "Starting publisherwebsite..."
    ssh $SERVER -p $SERVER_PORT "./publisherwebsite -port $PUBLISHERWEBSITE_PORT"
    ./publisherwebsite 
  else
